// MacroPage.jsx — con subtabs: Inflación · PBI · EMAE · Riesgo País
  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      [1,2,3,4,5,6].forEach(gv => {
        ...
        ctx.fillStyle = 'rgba(90,101,133,0.7)';
      ...
      slice.forEach((d, i) => {
        ...
        ctx.fill();
        if (isL) {
          ctx.fillStyle = 'rgba(91,156,246,0.9)';
          ctx.font = 'bold 9px JetBrains Mono, monospace';
          ctx.textAlign = 'center';
          ctx.fillText(v.toFixed(1).replace('.', ',') + '%', x + barW / 2, y - 5);
        }
            ...
            {byYear[yr].map((v, m) => {
              ...
              return (
                <div key={m} title={label}
                  style={{background:`rgba(${c.rgb},${alpha})`,borderRadius:'4px',height:'34px',display:'flex',alignItems:'center',justifyContent:'center',cursor:'default',border:`1px solid rgba(${c.rgb},${Math.min(1,alpha+0.1)})`,transition:'filter .12s'}}
...
function RiesgoPaisChart({ history, range }) {
  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      grad.addColorStop(0, 'rgba(224,92,92,0.16)');
      grad.addColorStop(1, 'rgba(224,92,92,0.01)');
      ...
      ctx.fillStyle = 'rgba(224,92,92,0.88)'; ctx.fill();
  ...
  const handleMouseMove = (e) => {
    ...
    const d = series[idx];
    if (d) setTooltip(`${d.fecha}  ·  ${Math.round(parseFloat(d.valor??0)).toLocaleString('es-AR')} pb`);
  };
...
function PbiBarChart({ history }) {
  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      ctx.scale(dpr, dpr);
      const W = rect.width, H = rect.height;
      const pad = { t: 28, r: 12, b: 40, l: 44 };
      const vals = slice.map(d => parseFloat(d.valor || 0));
      const maxV = Math.max(Math.max(...vals) * 1.25, 1);
      const minV = Math.min(Math.min(...vals) * 1.25, -1);
      const range = maxV - minV;
      const n = slice.length;
      const totalW = W - pad.l - pad.r;
      const barW = Math.floor(totalW / n * 0.70);
      const gap = totalW / n;
      ...
      slice.forEach((d, i) => {
        ...
        const isL = i === n - 1;
        const x = pad.l + i * gap + (gap - barW) / 2;
        const barH = Math.max(2, Math.abs(v) / range * (H - pad.t - pad.b));
        const y = pos ? zeroY - barH : zeroY;
        const grad = ctx.createLinearGradient(0, y, 0, y + barH);
        if (pos) {
          grad.addColorStop(0, isL ? 'rgba(74,191,120,0.88)' : 'rgba(74,191,120,0.52)');
          grad.addColorStop(1, 'rgba(74,191,120,0.04)');
        } else {
          grad.addColorStop(0, isL ? 'rgba(224,92,92,0.82)' : 'rgba(224,92,92,0.48)');
          grad.addColorStop(1, 'rgba(224,92,92,0.03)');
        }
        ctx.fillStyle = grad;
        ctx.beginPath();
        ...
        ctx.fill();
        if (isL) {
          ctx.fillStyle = pos ? 'rgba(74,191,120,0.9)' : 'rgba(224,92,92,0.85)';
          ctx.font = 'bold 9px JetBrains Mono,monospace'; ctx.textAlign = 'center';
          ctx.fillText((v >= 0 ? '+' : '') + v.toFixed(1).replace('.', ',') + '%', x + barW/2, pos ? y - 5 : y + barH + 12);
        }
        ...
        ctx.save(); ctx.translate(x + barW/2, H - pad.b + 9); ctx.rotate(-Math.PI/4);
  ...
  const handleMouseMove = (e) => {
    ...
    if (d) {
      const v = parseFloat(d.valor||0);
      const fp = (d.fecha||'').split('-');
      setTooltip(`Q${Math.ceil(+(fp[1]||3)/3)} ${fp[0]}  ·  ${v>=0?'+':''}${v.toFixed(1).replace('.', ',')}%`);
    }
  };
  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      items.forEach((item, i) => {
        ...
        ctx.globalAlpha = isH ? 1.0 : 0.82;
        ctx.fill(); ctx.globalAlpha = 1;
        ctx.strokeStyle = 'rgba(24,27,34,0.85)';
        ctx.lineWidth = isH ? 2 : 1.5;
        ctx.beginPath(); ctx.arc(cx, cy, rO, angle, angle + sweep); ctx.stroke();
        angle += sweep;
      });
      ...
      if (hovered != null && items[hovered]) {
        ...
        ctx.font = `bold ${Math.floor(r*0.34)}px JetBrains Mono, monospace`;
        ctx.fillText(it.share.toFixed(1) + '%', cx, cy - 8);
        ctx.fillStyle = 'rgba(138,151,184,0.8)';
        ctx.font = `${Math.floor(r*0.15)}px Inter, sans-serif`;
        ...
        lines.push(line);
        lines.slice(0,2).forEach((l,li) => ctx.fillText(l, cx, cy + 12 + li*13));
      } else {
        ctx.fillStyle = 'rgba(90,101,133,0.8)';
        ctx.font = `${Math.floor(r*0.18)}px JetBrains Mono, monospace`;
        ctx.fillText('% PBI', cx, cy - 4);
        ctx.font = `${Math.floor(r*0.13)}px JetBrains Mono, monospace`;
  ...
  const ipcLineData = (() => {
    ...
    if (mensData.length>=12) {
      ...
      for (let i=11;i<mensData.length;i++) {
        ...
        s.push({fecha:mensData[i].fecha,valor:(chunk.reduce((a,x)=>a*(1+parseFloat(x.valor||0)/100),1)-1)*100});
      }
      return s.slice(-24);
    }
    return [];
    ...
    <div>
      ...
      <div style={{display:'grid',gridTemplateColumns:'repeat(4,1fr)',gap:'12px',marginBottom:'20px'}}>
        <div style={{background:'var(--bg1)',border:'1px solid var(--line)',borderTop:`2px solid ${pbiVal!=null&&pbiVal>=0?'var(--green)':'var(--red)'}`,borderRadius:'10px',padding:'16px 18px'}}>
          <div style={{fontFamily:'var(--mono)',fontSize:'9px',letterSpacing:'.1em',textTransform:'uppercase',color:'var(--text3)',marginBottom:'8px'}}>
            Var. PBI real <span style={{background:pbiVal!=null&&pbiVal>=0?'var(--green-bg)':'var(--red-bg)',color:pbiVal!=null&&pbiVal>=0?'var(--green)':'var(--red)',borderRadius:'3px',padding:'1px 6px',marginLeft:'4px',fontSize:'8px'}}>{pbiTrim}</span>
          </div>
        ...
        <div style={{background:'var(--bg1)',border:'1px solid var(--line)',borderRadius:'10px',padding:'16px 18px'}}>
          ...
          <div style={{fontFamily:'var(--mono)',fontSize:'26px',fontWeight:700,color:'var(--white)',lineHeight:1}}>
            {pbiHist.length>=4?fmtPbi(pbiHist.slice(-4).reduce((s,d)=>s+parseFloat(d.valor||0),0)/4):'—'}
          </div>
        ...
        <div style={{background:'var(--bg1)',border:'1px solid var(--line)',borderRadius:'10px',padding:'16px 18px'}}>
          ...
          {(() => {
            ...
            return <><div style={{fontFamily:'var(--mono)',fontSize:'26px',fontWeight:700,color:'var(--green)',lineHeight:1}}>{fmtPbi(parseFloat(best.valor))}</div><div style={{fontFamily:'var(--mono)',fontSize:'10px',color:'var(--text3)',marginTop:'6px'}}>Q{Math.ceil(+m/3)} {y}</div></>;
          })()}
      ...
      <div style={{background:'var(--bg1)',border:'1px solid var(--line)',borderRadius:'10px',overflow:'hidden'}}>
        ...
        {pbiHist.length===0
          ...
          : pbiHist.slice(-8).reverse().map((d,i) => {
            const v=parseFloat(d.valor||0), pos=v>=0;
    ...
    <div>
      <div style={{display:'grid',gridTemplateColumns:'repeat(4,1fr)',gap:'12px',marginBottom:'20px'}}>
        <div style={{background:'var(--bg1)',border:'1px solid var(--line)',borderRadius:'10px',padding:'16px 18px'}}>
          ...
          <div style={{fontFamily:'var(--mono)',fontSize:'10px',color:emaeVal!=null&&emaeVal>=0?'var(--green)':'var(--red)',marginTop:'6px'}}>{emaeVal!=null?(emaeVal>=0?'crecimiento interanual':'contracción interanual'):'cargando…'}</div>
          <div style={{fontSize:'11px',color:'var(--text3)',marginTop:'4px'}}>Variación interanual</div>
        </div>
        <div style={{background:'var(--bg1)',border:'1px solid var(--line)',borderRadius:'10px',padding:'16px 18px'}}>
          <div style={{fontFamily:'var(--mono)',fontSize:'9px',letterSpacing:'.1em',textTransform:'uppercase',color:'var(--text3)',marginBottom:'8px'}}>Acum. {emaeAnioAc}</div>
          <div style={{fontFamily:'var(--mono)',fontSize:'28px',fontWeight:700,color:'var(--white)',lineHeight:1}}>{emaeAccum!=null?fmtE(emaeAccum):'—'}</div>
          <div style={{fontFamily:'var(--mono)',fontSize:'10px',color:'var(--text3)',marginTop:'6px'}}>{emae?.mesesAcum?`${emae.mesesAcum} mes${emae.mesesAcum>1?'es':''} disponibles`:'cargando…'}</div>
        ...
        <div style={{background:'var(--bg1)',border:'1px solid var(--line)',borderRadius:'10px',padding:'16px 18px'}}>
          ...
          <div style={{fontFamily:'var(--mono)',fontSize:'15px',fontWeight:700,color:'var(--green)',lineHeight:1.2}}>{bestSector?bestSector.nombre:'—'}</div>
          <div style={{fontFamily:'var(--mono)',fontSize:'14px',fontWeight:700,color:'var(--green)',marginTop:'4px'}}>{bestSector?fmtE(bestSector.valor):'—'}</div>
          <div style={{fontSize:'11px',color:'var(--text3)',marginTop:'4px'}}>{bestSector?.fecha?(() => { const [y,m]=bestSector.fecha.split('-'); return 'Interanual · '+(MESES_C[+m]||'')+' '+y; })():'Interanual'}</div>
        </div>
        <div style={{background:'var(--bg1)',border:'1px solid var(--line)',borderRadius:'10px',padding:'16px 18px'}}>
          <div style={{fontFamily:'var(--mono)',fontSize:'9px',letterSpacing:'.1em',textTransform:'uppercase',color:'var(--text3)',marginBottom:'8px'}}>{worstSector?.valor<0?'En contracción':'Menor crecimiento'}</div>
          <div style={{fontFamily:'var(--mono)',fontSize:'15px',fontWeight:700,color:worstSector?.valor<0?'var(--red)':'var(--text2)',lineHeight:1.2}}>{worstSector?worstSector.nombre:'—'}</div>
          <div style={{fontFamily:'var(--mono)',fontSize:'14px',fontWeight:700,color:worstSector?.valor<0?'var(--red)':'var(--text2)',marginTop:'4px'}}>{worstSector?fmtE(worstSector.valor):'—'}</div>
          <div style={{fontSize:'11px',color:'var(--text3)',marginTop:'4px'}}>{worstSector?.valor<0?'único o entre negativos':'menor ritmo de expansión'}</div>
        </div>
      </div>
      ...
      <div style={{display:'grid',gridTemplateColumns:'1fr 300px',gap:'14px'}}>
        ...
        <div style={{background:'var(--bg1)',border:'1px solid var(--line)',borderRadius:'10px',overflow:'hidden',display:'flex',flexDirection:'column'}}>
          ...
          {regionalLoading&&!Object.keys(regionalData).length ? (
            ...
            const barColor=pb>=350?'rgba(224,92,92,0.50)':pb>=200?'rgba(143,184,240,0.50)':'rgba(74,191,120,0.45)';
            return (
    ...
    <div className="page-enter">
      ...
      <div className="ph">
        ...
        <div className="ph-right" style={{fontFamily:'var(--mono)',fontSize:'11px',color:'var(--text3)'}}>
          INDEC · BCRA · JP Morgan
        </div>
      </div>

      {/* KPI resumen — clickeable para ir a tab */}
      <div className="section">
        ...
        <div className="grid grid-4">
          {[
            ...
            { lbl:'EMAE General', badge:emaeMesKpi, val:fmt1(emaeVal), sub:emae?.acumAnio!=null?`Acum. ${(emae.acumAnio>=0?'+':'')+emae.acumAnio.toFixed(1).replace('.',',')}% en ${emae.anoAcum}`:'—', meta:'Var. interanual · INDEC', tab:'emae' },
            ...
            { lbl:'PBI Real', badge:pbiTrim, val:fmtPbi(pbiVal), sub:pbi?.prevIa!=null?`Trim. ant.: ${fmtPbi(pbi.prevIa)}`:'—', meta:'Var. interanual · INDEC', tab:'pbi' },
          ].map((k,i) => (
