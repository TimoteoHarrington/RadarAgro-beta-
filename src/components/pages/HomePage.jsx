// HomePage.jsx — widget grid idéntico al HTML de referencia
        ...
        <div style={{ marginBottom: '16px' }}>
          <div className="edit-bar">
            ...
            <div style={{ display: 'flex', gap: '8px', marginLeft: 'auto', alignItems: 'center' }}>
              <button onClick={() => setShowCatalog(v => !v)} style={{ background: 'var(--bg3)', border: '1px solid var(--line2)', color: 'var(--text2)', fontFamily: 'var(--body)', fontSize: '12px', padding: '7px 16px', borderRadius: '6px', cursor: 'pointer', fontWeight: 500 }}>
                + Agregar widget
              </button>
              <button onClick={() => { setEditMode(false); setShowCatalog(false); }} style={{ background: 'var(--accent)', border: 'none', color: '#fff', fontFamily: 'var(--body)', fontSize: '12px', padding: '7px 20px', borderRadius: '6px', cursor: 'pointer', fontWeight: 600 }}>
                Listo
              </button>
            </div>
          </div>
            ...
            >
              {editMode && (
                <>
                  ...
                  <button onClick={() => removeWidget(w.id)} style={{ position: 'absolute', top: '6px', right: '8px', zIndex: 30, background: 'none', border: 'none', cursor: 'pointer', color: 'var(--red)', fontSize: '16px', lineHeight: 1 }} title="Ocultar">✕</button>
                  <div style={{ position: 'absolute', bottom: '8px', right: '8px', zIndex: 25, display: 'flex', gap: '3px' }}>
                    {allowed.map(s => (
                      <button key={s} onClick={() => setWidgetSize(w.id, s)} className={`wcat-sz${size === s ? ' sel' : ''}`} style={{ fontSize: '8px', padding: '3px 7px' }}>{SIZE_LABELS[s]}</button>
                    ))}
                  </div>
